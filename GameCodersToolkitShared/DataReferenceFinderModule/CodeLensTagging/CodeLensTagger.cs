	...
	internal sealed class CodeLensTagger : ITagger<ICodeLensTag>
	{
		...
		public IEnumerable<ITagSpan<ICodeLensTag>> GetTags(NormalizedSnapshotSpanCollection spans)
		{
			...
			foreach (var guidResultLine in m_guidResultLines)
			{
				...
				SnapshotSpan tagSpan = new SnapshotSpan(lineSnapshot.Start,
					lineSnapshot.End);
				codeLensTag.Descriptor = new DataReferenceCodeLensDescriptor
				{
		...
		private void ParseBufferSpan(ITextSnapshot oldSnapshot, ITextSnapshot newSnapshot, Span oldSpan, Span newSpan, int lineDelta)
		{
			...
			List<string> guidFieldIdentifiers = new List<string>();
			guidFieldIdentifiers = GameCodersToolkitPackage.DataLocationsConfig.GetGuidFieldIdentifiers();

			Dictionary<string, string> identifierToGuid = new Dictionary<string, string>();
			Dictionary<string, List<int>> identifiersToResolve = new Dictionary<string, List<int>>();

			SnapshotSpan newSnapshotSpan = new SnapshotSpan(newSnapshot, newSpan);
			int newStartLine = newSnapshotSpan.Start.GetContainingLine().LineNumber;
			int newEndLine = newSnapshotSpan.End.GetContainingLine().LineNumber;
			...
			{
				ITextSnapshotLine lineSnapshot = newSnapshot.GetLineFromLineNumber(lineToParse);
			...
			if (lineDelta != 0)
			{
				...
				{
					if (m_guidResultLines[i].LineNumber >= newStartLine)
					{
